...
const reducer = (state: IBookLibraryState, action: Action): IBookLibraryState => {
	switch (action.type) {
		...
		case 'REMOVE_DIARY_ENTRY':
			return {
				...state,
				readingDiary: state.readingDiary.filter((entry) => entry.id !== action.payload),
			}
		...
		case 'REMOVE_ONE_TOAST_MESSAGE':
			return {
				...state,
				toastMessagePipeline:
					state.toastMessagePipeline.length > 0 ? state.toastMessagePipeline.slice(1) : [],
			}
...
export const BookLibraryContext = createContext({} as IBookLibraryContext)
...
export const BookLibraryProvider = ({ children }: ContextProviderProps) => {
	...
	const reorderEntries = (filterType: FilterTypes, contentType: ContentType) => {
		switch (contentType) {
	...
	const removeReadingDiaryEntry = (entryId: string | number) => {
		...
		localStorage.setItem(
			'readingDiary',
			JSON.stringify(parsedDiary.filter((entry: ReadingDiaryEntry) => entry.id !== entryId))
		)
...
export const useBookLibraryContext = () => {
	...
	if (!context) {
		throw new Error('useBookLibraryContext deve ser usado dentro de um BookLibraryContext')
	}
	return context
}
