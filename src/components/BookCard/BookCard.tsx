...
const BookCard: FC<IBookCard> = ({ book }) => {
	...
	return (
		<article
			...
			className={[styles.container, checkForLocation === 0 && styles.isOnHome].join(' ')}
		>
			<span
				onClick={() => {
					if (checkForLocation !== 0) handleButtons(BookCardButtons.ProductPage)
				}}
			...
			<div className={styles.container_textsDiv}>
				...
				{checkForLocation === 0 ? null : checkForLocation === 1 ? (
					<button
						...
						className={styles.container_textsDiv__productRemoveFromLibraryButton}
					>
						Remover
					</button>
				) : (
