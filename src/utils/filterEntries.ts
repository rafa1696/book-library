...
const filterEntries = (
	entries: BookInfo[] | ReadingDiaryEntry[] | undefined | null,
	...
	const isBook = (entries: BookInfo[] | ReadingDiaryEntry[]): entries is BookInfo[] => {
		return (entries as BookInfo[])[0].bookName !== undefined
	}

	const getProperty = (entry: BookInfo | ReadingDiaryEntry, property: string) => {
		// TODO - Consertar any
	...
	switch (howToReorder) {
		...
		case FilterTypes.Oldest:
			return entries.sort((a, b) => getProperty(a, dateProperty) - getProperty(b, dateProperty))
		case FilterTypes.Newest:
			return entries.sort((a, b) => getProperty(b, dateProperty) - getProperty(a, dateProperty))
		default:
			return entries
	}
}
